...
const Bento = () => {
    ...
    return (
        <div className='h-full w-full flex items-start justify-start p-1'>
            <div className="grid grid-cols-1 grid-rows-5 md:grid-cols-4 md:grid-rows-3 w-full h-[50%] gap-4 xl:m-15 xl:mx-52 lh:m-34 md:m-16">
                {bentoCards.map((card, index) => (
                    <div 
                        ...
                        className={`rounded-2xl transition-color duration-300 ease-in-out group flex items-end overflow-hidden h-full bg-gray-500
                            ...
                            `}
                    >
                        <div className='flex flex-col mx-auto h-full w-full justify-end bg-opacity-100 relative z-10 overflow-hidden transition-all duration-500 ease-in-out p-4 group-hover:bg-gray-300/10'>
                            <h2 className='font-bold text-lg/tight tracking-tight text-white max-w-[70%] transform ease-in-out transition-all duration-300 delay-100'>
                                {card.title}
                            </h2>
